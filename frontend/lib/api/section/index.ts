import { SectionCreate, SectionUpdate, SectionResponse } from '@/types/lms';
import api from '../../api';
...
export async function createSection(section: SectionCreate): Promise<SectionResponse> {
  const res = await api.post<SectionResponse>('/api/section', section);
  return res.data;
}
...
export async function updateSection(id: number, section: SectionUpdate): Promise<SectionResponse> {
  const res = await api.put<SectionResponse>(`/api/section/${id}`, section);
  return res.data;
}
